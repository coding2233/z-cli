...
void CliApp::Initialize(std::string app_path)
{
    ...
    // 设置虚拟文件系统
    SetupVirtualFileSystem(app_directory);
...
void CliApp::SetupVirtualFileSystem(const std::string& app_directory)
{
    // 创建本地文件系统，指向程序所在目录  
    ...
    SPDLOG_INFO("Virtual file system initialized with app directory: {}", app_directory);
}

void CliApp::LoadConfiguration()
{
    auto& config = ConfigManager::GetInstance();
    config.LoadConfig();
    SPDLOG_INFO("Configuration loaded");
}

void CliApp::InitializeLogging()
{
    auto& config = ConfigManager::GetInstance();
    std::string log_level = config.GetString("app.log_level", "info");
    ...
    SPDLOG_INFO("Logging initialized with level: {}", log_level);
}